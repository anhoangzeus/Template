...
export default class Cart extends Component {
  ...
  GetAddress = () => {
    if (auth().currentUser) {
      this.itemRef.ref('ListAddress').child(auth().currentUser.uid).once('value').then((snapshot) => {
        var item;
  ...
  _tangSoLuong = (item) => {
    this.itemRef.ref('Cart/' + auth().currentUser.uid + "/" + item.Id).set({
      Id: item.Id,
      Name: item.Name,
  ...
  _xoaGioHang = () => {
    this.itemRef.ref('Cart/' + auth().currentUser.uid).child(this.state._idCanXoa).remove();
    this.ListenCart();
    this.setState({
      modalVisible: false,
      refesh: !this.state.refesh
      ...
      <View style={styles.itemcard}>
        ...
        <View style={styles.itemInfo}>
          ...
          <View style={styles.itemDec}>
            ...
            <View style={{ flexDirection: "row" }}>
              <TouchableOpacity style={styles.buttonUpDown} onPress={() => { this._giamSoLuong(item) }}>
                <Text style={{color:"#fff",textAlign:"center",fontSize:20,fontWeight:'bold'}}>-</Text>
              </TouchableOpacity>
            <View style={{ marginTop: 10 }}><Text style={{ fontSize: 20, marginHorizontal: 20, color: '#000' }}>{item.Quantity}</Text></View>
            <TouchableOpacity title="+" style={styles.buttonUpDown} onPress={() => { this._tangSoLuong(item) }}>
            <Text style={{color:"#fff",textAlign:"center",fontSize:20,fontWeight:'bold'}}>+</Text>
            </TouchableOpacity>
          </View>
        </View>
